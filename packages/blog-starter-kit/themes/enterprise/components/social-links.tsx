...
export const SocialLinks = ({ isSidebar, isTopMenu }: SocialLinksProps) => {
	...
	return (
		...
		>
			<div
				className={`grid gap-0.5 ${isTopMenu ? 'grid-cols-8' : isSidebar ? 'grid-cols-4' : 'grid-cols-3 lg:grid-cols-4'}`}
			>
				{hasSocialLinks && publication.links?.twitter && (
					<a
				...
				{hasSocialLinks && publication.links?.github && (
					<a
				...
				{hasSocialLinks && publication.links?.linkedin && (
					<a
				...
				{hasSocialLinks && publication.links?.hashnode && (
					<a
